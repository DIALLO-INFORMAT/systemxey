...
global $pdo, $user_id_entreprise, $offre_a_modifier; // $offre_a_modifier est chargé si mode=modifier

$mode = $_GET['mode'] ?? 'creer'; // 'creer' ou 'modifier'
$form_action = ($mode === 'modifier' && isset($offre_a_modifier['id'])) ? 'modifier_offre' : 'creer_offre';
$form_title = ($mode === 'modifier' && isset($offre_a_modifier['id'])) ? 'Modifier l\'Offre d\'Emploi' : 'Publier une Nouvelle Offre d\'Emploi';

// Pré-remplir les champs si en mode modification ou si erreur de validation
$titre_val = $_POST['titre_poste'] ?? ($offre_a_modifier['titre_poste'] ?? '');
$desc_val = $_POST['description_poste'] ?? ($offre_a_modifier['description_poste'] ?? '');
$type_val = $_POST['type_contrat'] ?? ($offre_a_modifier['type_contrat'] ?? '');
$lieu_val = $_POST['lieu'] ?? ($offre_a_modifier['lieu'] ?? '');
$comp_val = $_POST['competences_requises'] ?? ($offre_a_modifier['competences_requises'] ?? '');
$date_lim_val = $_POST['date_limite_candidature'] ?? ($offre_a_modifier['date_limite_candidature'] ?? '');
$active_val = isset($_POST['est_active_offre']) ? $_POST['est_active_offre'] : ($offre_a_modifier['est_active'] ?? 1);

?>
<h2><i class="fas <?php echo ($mode === 'modifier') ? 'fa-edit' : 'fa-plus-circle'; ?>"></i> <?php echo $form_title; ?></h2>
